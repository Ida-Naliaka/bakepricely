...
const RecipeTemplate = ({passedrecipe, reload}) => {
  ...
  const { totalCost, setTotalCost, setReload} = CostState();
  ...
  const handlePresavedUnchanged = (recipeIngredients) => {
    ...
    if(ingredientsarr) {
      ...
      let obj = {};
      //in case of 300ml instead of 300 ml
    ...
    calculateIngredientsCost(toUse, user).then((costOfIngredients)=>{
      const totalInclusive= Number(costOfIngredients + recipe.LabourCost * recipe.LabourHours + recipe.Overheads);
      const profit = Number(recipe.Profit / 100) * Number(totalInclusive);
  ...
  useEffect(() => {
    if (passedrecipe && passedrecipe.content) {
      setRecipe(passedrecipe)
      handlePresavedUnchanged(passedrecipe.content);
      setContent(passedrecipe.content);
      setServings(passedrecipe.servings);
      setLabourCostPerHour(passedrecipe.LabourCost)
      setHours(passedrecipe.LabourHours)
      setPercentageProfit(passedrecipe.Profit);
      setOverheadCost(passedrecipe.Overheads)
    } else {
  ...
  useEffect(() => {
    if (recipe && (
      recipe.LabourCost===labourCostPerHour &&
      recipe.servings===servings &&
      recipe.LabourHours===hours &&
      recipe.Overheads===overheadCost&&
      recipe.Profit===percentageProfit &&
  ...
  //eslint-disable-next-line
  }, [labourCostPerHour, hours, overheadCost, percentageProfit, content, servings]);
  ...
  const handleIngredients = async (recipeIngredients) => {
    ...
    for (let i = 0; i < ingredientsarr.length; i++) {
      let obj = {};
      if (isRecipeIngredient(ingredientsarr[i])) {
        const component = ingredientsarr[i].split(" ");
        if (component.length > 2) {
          const itemName = component.slice(2, component.length).join(" ");
          obj["quantity"] = component[0];
          obj["measure"] = component[1];
  ...
  const handleCalculateCost = async () => {
    ...
    await axios
      ...
      .then((res) => {
        ...
        let saved;
        res.data.nofolderfiles.map((file) => {
          if (file._id === recipe._id) {
            saved = file;
          }
        });
        if (saved) {
  ...
  useEffect(() => {
    if ((servings, content && labourCostPerHour && overheadCost &&percentageProfit && hours) && changed) {
      const delayFn = setTimeout(() => handleCalculateCost(), 3000);
      return () => clearTimeout(delayFn);
    }
    //eslint-disable-next-line
  }, [servings, content, labourCostPerHour, overheadCost, percentageProfit, hours, changed]);
  
  return (
    <div className="flex flex-col items-start ml-[5vw] md:mt-11">
      <h2 className="font-bold text-[32px]">{recipe? recipe.name.charAt(0).toUpperCase() + recipe.name.slice(1): "New Recipe"} </h2>
